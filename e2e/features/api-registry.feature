Feature: WUD Registry API Exposure
  ...
  Scenario: WUD must allow to get all Registries
    ...
    And response body path $[1].id should be hub
    And response body path $[1].type should be hub
    And response body path $[1].name should be hub
    And response body path $[1].configuration.login should be fmartinou
    And response body path $[1].configuration.token should be .\*.*.
